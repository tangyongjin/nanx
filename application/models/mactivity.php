...
class MActivity extends CI_Model
{
    ...
    function getFieldesByType($activity_summary, $para_array){
        ...
        if (($activity_type == 'table') || ($base_table == 'nanx_code_table')) {
            ...
            if ($fields_e_with_idforbidden_option['id_hidden']) {
                $id_col_cfg                               = $this->MFieldcfg->getColsCfg($activity_code, $base_table, array(
                    'id'
                ), $para_array['transfer']);
    ...
    function getBpmCfg($para_array){
         $allsession = $this->session->all_userdata();
         $roles         = $allsession['roles'];
         foreach ($roles as $key => $onerow) {
           $where=array('activity_code'=>$para_array['code'],'role_code'=>$onerow['role_code']);
           $row=$this->db->get_where('nanx_user_role_privilege',$where)->row_array();
           if($row){
               return $row;         
           }
        }
    ...
    function getActivityCfg($para_array){
        ...
        $activity_summary['bpmcfg']             = $this->getBpmCfg($para_array); 
